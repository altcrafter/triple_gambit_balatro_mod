--[[
    ...
    Initializes TG. Loads modules. Sets up 4 boards.
    Phase 1: per-board resources, keys-only deck, no ResourcePool, no gambits.lua.
]]
...
TG.CONFIG = {
    ...
    -- Game-logic colors (distinct from UI broadcast colors in ui/ files)
    COLORS = {
...
function TG:init()
    ...
    print("[TG] Initializing Triple Gambit (Phase 1 — 4 boards)...")
    ...
    -- NOTE: resource_pool.lua deleted (per-board resources now in board.lua)
    -- NOTE: gambits.lua deleted (duplicate of gambit.lua)
    -- NOTE: gambit_base.lua moved to dormant/
    ...
    print("[TG] Triple Gambit ready. Four boards initialized (A, B, C, D).")
end
...
-- ============================================================
-- RESOURCE SYNC
-- Write the active board's resources into Balatro's counters.
-- ============================================================
...
function TG:on_blind_start()
    if TG.Amplifier then TG.Amplifier.reset() end

    -- Sync ante/blind type from Balatro
...
function TG:on_blind_end()
    ...
    local board = self:get_active_board()
    if board then
        local reward = board.hands_remaining * (TG.CONFIG.REWARD_PER_UNUSED_HAND or 1)
